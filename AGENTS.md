...
npm run setup:demo
npm run discover
npm run draft -- --task-id task-0001 --title "任务标题" --body "任务正文" --type documentation --level junior
...
- 接到用户任务后，优先使用 `npm run draft -- ...` 生成 `employee-task.v1` 草稿，再使用 `npm run intake -- --file ...` 自动选择、派发并触发员工。
- `intake` 默认使用员工 `--auto-run` 非交互模式；只有调试时才加 `--interactive`。
- 需要多个员工并行时，使用 `npm run intake -- --file ... --background`，并用 `npm run runs` 观察运行状态。
- `npm run runs` 默认只读；只有需要刷新聚合索引时才使用 `npm run runs -- --refresh`。
- 后台 worker 异常退出或怀疑遗漏收集时，使用 `npm run sweep` 补跑 `collect/reconcile`，不要自动重试员工任务。
